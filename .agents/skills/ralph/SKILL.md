...
当前项目是 Go 本地工具仓，不是内容项目、Web 应用或后端 SaaS。转换 PRD 时默认遵守：

- `project` 固定为 `chatlog_alpha`，`branchName` 使用 `ralph/<feature-name-kebab-case>`。
- story 必须小到一次 Ralph 迭代能完成，并且只能覆盖一个 CLI、HTTP、daily report、semantic、temporal graph、Hermes push、harness 或文档行为。
- acceptance criteria 优先写具体命令和可观测结果，例如 `./init.sh`、focused `go test ./internal/...`、`go run . http list`、`curl .../api/v1/ping`、文件路径/时间戳/计数/状态码。
- 涉及私密聊天数据时，只要求验证路径、时间戳、文件大小、结构化计数、HTTP 状态、错误桶或高层字段；不要要求输出消息正文或报告正文。
- 默认禁止把 `chatlog report daily --vision`、`chatlog report daily --summary` 写进验收，除非 PRD 和用户授权明确要求模型/vision 调用。
- 不要把泛化 UI/browser 标准塞进非 UI story；只有明确涉及运行中页面时才写 agent-browser 验收。
